...
class RssStore:
    ...
    def add_feed(self, chat_id: int | str, url: str):
        cid = str(chat_id)
        with self.lock:
            chat = self.data.setdefault("chats", {}).setdefault(cid, {"feeds": [], "last_digest_ts": 0.0, "pending": {}})
            if url not in chat["feeds"]:
                chat["feeds"].append(url)
            self.data.setdefault("feeds_meta", {}).setdefault(url, {"etag": None, "last_modified": None, "seen_ids": []})
    ...
    def get_feed_meta(self, url: str) -> Dict[str, Any]:
        with self.lock:
            return dict((self.data.setdefault("feeds_meta", {}).setdefault(url, {"etag": None, "last_modified": None, "seen_ids": []})))

    def update_feed_meta(self, url: str, etag: Optional[str], last_modified: Optional[str]):
        with self.lock:
            meta = self.data.setdefault("feeds_meta", {}).setdefault(url, {"etag": None, "last_modified": None, "seen_ids": []})
            if etag is not None:
                meta["etag"] = etag
